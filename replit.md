...
- **Cash & Card Payment System**: Riders can pay via Cash, Wallet, or Card. Cash trips: rider pays driver directly, driver collects 100% of fare, platform share deferred to `platform_settlements` table for async settlement. Card/wallet trips: handled internally with immediate wallet credits. Driver wallet page shows collapsible settlement summary for outstanding cash trip balances. Trip cards and detail modals show "Cash collected" for cash trips. Variable driver commission (70-80%) tracked via `driver_standing` table based on trips completed, rating, and account age. Settlement API endpoints for driver summary, pending settlements, and admin management.
