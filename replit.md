...
- **Multi-Language Support (i18n)**: 12 languages (English, French, Arabic, Hausa, Igbo, Yoruba, Swahili, Zulu, Xhosa, Afrikaans, Portuguese, Spanish) with LanguageProvider context, RTL support for Arabic, language selection page at /rider/settings/language, translations integrated across all key Rider screens (Home, Services, Activity, Inbox, Account, Settings, Payments), backend API for language persistence (GET/POST /api/user/language), and draggable ZIBA Support floating button with localStorage position persistence and edge-snapping. Safety mechanisms include English reset buttons on the language page, always-visible "Reset to English" text, and ZIBRA auto-response for language help keywords.
