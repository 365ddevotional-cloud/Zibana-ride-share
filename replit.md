...
- **Director Dashboard (Private)**: Dedicated director dashboard isolated from admin dashboard. Shows profile overview, lifespan countdown, cell tabs with per-cell metrics, staff list, coaching alerts, and action logs. Accessible to director, admin, and super_admin only.
...
- **Director Audit & Accountability**: Enhanced action logging capturing actorId, role, action, target, timestamp, and before/after state for all director governance actions. Logs visible to admin and super_admin only.
