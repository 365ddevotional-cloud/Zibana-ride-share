...
### UI/UX
The application features role-based dashboards, a public landing page, and supports both dark and light modes. It incorporates lazy loading, error boundaries, and loading skeletons for an optimized user experience. The UI is built to support configurable app modes (RIDER/DRIVER/UNIFIED) from a single codebase, utilizing React 18 with TypeScript, Wouter for routing, and Tailwind CSS integrated with shadcn/ui components. The Admin dashboard uses an **Executive Visual Theme** with dark navy sidebar (#0F172A), blue-600 active highlights, color-coded KPI cards with top borders and background icons, section cards with left accent stripes, shadow-lg polish, and a max-w-7xl content container. Super Admin badge uses emerald styling. Typography uses tracking-tight headings and slate-based text hierarchy.
...
- **Admin Control Center**: Dedicated routable section at `/admin/control/:section` with 5 sub-pages: Overview (KPIs, system status, attention items, section navigation), Monitoring (active trips table with search/filter, driver status table with search), Health Alerts (reuses HealthAlertsPanel), Launch Readiness (reuses LaunchReadinessPanel with kill switches and country management), and Ops Readiness (reuses OperationalReadinessPanel with playbooks). Features breadcrumb navigation, section tab bar, back-to-admin button, and consistent executive theme. Key files: client/src/pages/admin/control-center-layout.tsx (layout/routing), client/src/pages/admin/control-overview.tsx (overview page), client/src/pages/admin/control-monitoring.tsx (monitoring page). Routes: /admin/control/overview, /admin/control/monitoring, /admin/control/alerts, /admin/control/launch, /admin/control/ops.
