...
- **Director Lifecycle Management**: Extended `directorProfiles` with `maxCells`, `createdBy`, `lastModifiedBy`, `lastModifiedAt` fields. Endpoints for terminate (super_admin only), GET lifecycle detail (admin), enhanced suspend/reactivate with audit logging and IP capture. Admin lifecycle detail dialog with lifespan controls, status transitions, and recent activity.
