...
- **Ratings, Behavior Signals & Trust Scoring**: Implements a one-time, bidirectional rating system (1-5 stars) within a 72-hour window. All new users start with 5.0 star rating (trust profile auto-created on signup). Captures passive behavior signals (e.g., `GPS_INTERRUPTION`, `NO_SHOW`) and positive signals. Calculates a trust score (0-100) based on ratings, behavior signals, trip completion ratio, and account age, with anti-manipulation guards. Pairing blocks are automatically created when rating <3 stars (mutual blocking), enforced in matching engine and ride acceptance. Super Admin can adjust ratings and remove pairing blocks via Admin UI, with full audit trail.
...
- **Admin Override Control & Support Safety**: Provides authorized admin/super_admin users with controlled override capabilities to fix user issues without engineering intervention. Supports 9 override action types: FORCE_LOGOUT, RESET_SESSION, RESTORE_AUTO_LOGIN, ENABLE/DISABLE_DRIVER_ONLINE, CLEAR_CANCELLATION_FLAGS, RESTORE_DRIVER_ACCESS, CLEAR_RIDER_CANCELLATION_WARNING, RESTORE_RIDE_ACCESS. All overrides are auditable via immutable `admin_override_audit_log`. Temporary overrides support expiration timestamps with auto-revert via 60-second scheduler. Safety rules prevent account deletion, trip history resets, or rating alterations. Admin UI "Overrides" tab shows active overrides, user history search, and full audit trail with confirmation dialogs.
- **New vs Returning User Analytics (Phase 4)**: Tracks real growth, retention, and platform health for riders and drivers. `user_analytics` table stores per-user session counts, first/last session timestamps, first ride/trip timestamps, activation status, and activity metrics. Session heartbeat fires once per app load via `useAuth` hook. Analytics hooks integrated into trip completion and driver online toggle. Admin "User Growth" tab shows: new/activated/returning users today, D1/D7/D30 rider and driver retention, average time to first ride/trip, rider funnel (signup → first ride → second ride), driver funnel (signup → approved → first trip → consistent activity), activity metrics per role, and automated decision signals for onboarding/pricing/availability issues.
- **Driver Acquisition Automation (Phase 9)**: Implements structured driver supply growth with four acquisition channels (REFERRAL, FLEET_OWNER, PUBLIC_SIGNUP, ADMIN_INVITED) and five onboarding stages (SIGNUP → DOCUMENTS → REVIEW → FIRST_TRIP → ACTIVE). Features referral rewards requiring 5 trips within 30 days for payout eligibility. Fleet owner management with configurable max drivers (default 50), per-activation bonuses, and suspension controls. Supply alerts trigger when zones have low driver availability with severity levels (LOW, MEDIUM, HIGH, CRITICAL). Zone-level acquisition controls allow pausing/resuming driver signup per country and state/region. Automated messaging at each onboarding stage. Admin "Acquisition" tab shows analytics (total acquired, by channel, avg time to first trip, referral conversion rate, fleet effectiveness, D7/D30 retention), onboarding pipeline visualization, driver pipeline table with approve action, fleet owner management, supply alerts, and zone control dialogs.
- **In-App Q&A & Help Center (Phase 10A)**: Implements a full-featured help center with FAQ system accessible to riders (`/rider/help`), drivers (`/driver/help`), and admins. `help_categories` table with slug, icon, audience targeting (ALL/RIDER/DRIVER/ADMIN), sort order, and active toggle. `help_articles` table with status workflow (DRAFT/PUBLISHED/ARCHIVED), audience targeting, featured flag, helpful ratings (yes/no), view counts, tags, country-specific content, and category association. `help_search_logs` table tracks user search queries and result counts for analytics. Public API supports category browsing, article search (title/summary/content), article viewing with auto-incrementing view count, and helpful/not-helpful rating. Admin "Help Center" tab (accessible to super_admin, admin, support_agent) provides CRUD management for categories and articles, status/audience/category filtering, and search log analytics.
