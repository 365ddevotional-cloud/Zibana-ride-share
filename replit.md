...
- **Director Payout Summaries & Controls**: `directorPayoutSummaries` table with full state machine (calculating/pending_review/approved/scheduled/released/held/reversed), period tracking (periodStart/periodEnd), eligibility snapshots, partial release amounts, ZIBRA flagging, and complete dispute workflow (submit/review/resolve). Auto-hold triggers when director is suspended/expired/commission-frozen. Admin payout actions: approve, schedule, release, partial_release, hold, reverse (Super Admin: create, force_release, force_hold). Director earnings tab shows payout states, last payout date, hold notices, dispute submission UI, and payout history. Admin panel shows full lifecycle with flags, states, dispute status, and action buttons. All actions logged with audit trail. ZIBRA oversight signals monitor held/reversed payouts, pending disputes, and frozen commissions. Endpoints: `GET /api/director/earnings`, `GET /api/admin/directors/:id/earnings`, `POST /api/admin/directors/:id/payout`, `POST /api/director/payout/:id/dispute`, `POST /api/admin/directors/:id/payout/:id/resolve-dispute`.
