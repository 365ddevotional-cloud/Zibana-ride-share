...
const UsersTrainingDetailPage = lazy(() => import("@/pages/admin/users-training-detail"));
...
const LiveDriverMap = lazy(() => import("@/pages/admin/live-map"));
...
const DriverPreferencesPage = lazy(() => import("@/pages/driver/preferences"));
...
const TripReceiptPage = lazy(() => import("@/pages/rider/trip-receipt"));
...
function DriverRouter() {
  ...
  const activatingRole = useRef(false);
  ...
  useEffect(() => {
    if (userRole && !roleLoading && user && !isRegisterPage) {
      ...
      if (hasDriverRole && userRole.role !== "driver" && !activatingRole.current) {
        ...
        fetch(`${API_BASE}/api/user/active-role`, {
          ...
          queryClient.invalidateQueries({ queryKey: ["/api/user/role"] });
          activatingRole.current = false;
        }).catch(() => {
          activatingRole.current = false;
        });
      }
    }
  }, [userRole, roleLoading, user, isRegisterPage]);
  ...
  return (
    <Switch>
      ...
      <Route path="/driver/preferences">
        <LazyComponent><DriverPreferencesPage /></LazyComponent>
      </Route>
...
function AdminRouter() {
  ...
  return (
    <Switch>
      ...
      <Route path="/admin/users/training-center/:slug">
        {(params: { slug: string }) => (
          <LazyComponent>
      ...
      <Route path="/admin/live-map">
        <LazyComponent>
          <AdminLayout userRole={role || "admin"} activeTab="live-map">
            <LiveDriverMap />
          </AdminLayout>
        </LazyComponent>
      </Route>
      ...
      <Route path="/admin/users/drivers/:id">
        {(params: { id: string }) => (
          <LazyComponent>
            <AdminLayout userRole={role || "admin"} activeTab="drivers">
              <AdminDriverDetail userId={params.id} />
            </AdminLayout>
          </LazyComponent>
        )}
      </Route>
      <Route path="/admin/users/riders/:id">
        {(params: { id: string }) => (
          <LazyComponent>
            <AdminLayout userRole={role || "admin"} activeTab="riders">
              <AdminRiderDetail riderId={params.id} />
            </AdminLayout>
          </LazyComponent>
        )}
      </Route>
...
function AuthenticatedRoutes() {
  ...
  const { data: userRole, isLoading: roleLoading } = useQuery<{ role: string; roles?: string[]; roleCount?: number } | null>({
    queryKey: ["/api/user/role"],
    enabled: !!user,
    retry: false,
  });
...
function RiderRouter() {
  ...
  return (
    <Switch>
      ...
      <Route path="/rider/trips/:tripId/receipt">
        <ProtectedRoute user={user} userRole={userRole} isLoading={isLoading} allowedRoles={["rider"]}>
          <LazyComponent><TripReceiptPage /></LazyComponent>
        </ProtectedRoute>
      </Route>
